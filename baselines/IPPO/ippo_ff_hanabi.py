...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _reset_count(train_state, reset_type):
                inner_state = train_state.opt_state[1]
                if reset_type == "count":
                    inner_state = (inner_state[0]._replace(count=0), inner_state[1])
                elif reset_type == "all":
                    inner_state = jax.tree_map(jnp.zeros_like, inner_state)
                opt_state = (train_state.opt_state[0], inner_state)
            ...
            train_state = _reset_count(train_state, config["RESET_TYPE"])
            update_state = (train_state, traj_batch, advantages, targets, rng)
